...
import { X, Sparkles, User, Users, Palette, Trash2, Plus, Check, Edit2, BookOpen, Upload, Wand2 } from 'lucide-react';
import {
...
export const ImaginariumSettingsModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      <div
        className="relative w-full max-w-sm rounded-[2rem] p-5 shadow-2xl z-10 space-y-4 max-h-[85dvh] flex flex-col backdrop-blur-2xl"
        style={{
      ...
      >
        <div className="flex items-center justify-between shrink-0 pb-2 border-b" style={{ borderColor: 'var(--card-border)' }}>
          <h3 className="font-serif font-bold text-sm tracking-wide">沙龙手稿与定制卡</h3>
        ...
        {/* 拟物 Card Tab */}
        <div className="grid grid-cols-4 gap-1 p-1 rounded-2xl shrink-0" style={{ backgroundColor: 'var(--control-soft-bg)' }}>
          <button
            type="button"
            onClick={() => setActiveTab('settings')}
            className={`py-1.5 rounded-xl text-[10px] font-bold transition-all flex flex-col items-center gap-0.5 ${activeTab === 'settings' ? 'shadow-sm' : 'opacity-60'}`}
          ...
          <button
            ...
            className={`py-1.5 rounded-xl text-[10px] font-bold transition-all flex flex-col items-center gap-0.5 ${activeTab === 'persona' ? 'shadow-sm' : 'opacity-60'}`}
          ...
          <button
            ...
            className={`py-1.5 rounded-xl text-[10px] font-bold transition-all flex flex-col items-center gap-0.5 ${activeTab === 'members' ? 'shadow-sm' : 'opacity-60'}`}
          ...
          <button
            ...
            className={`py-1.5 rounded-xl text-[10px] font-bold transition-all flex flex-col items-center gap-0.5 ${activeTab === 'summaries' ? 'shadow-sm' : 'opacity-60'}`}
            style={{ backgroundColor: activeTab === 'summaries' ? 'var(--card-bg)' : 'transparent' }}
          >
            <BookOpen className="w-3.5 h-3.5" /> 多阶段总结
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-3 text-xs">
          {activeTab === 'settings' && (
            <div className="space-y-3">
              {/* 卡片 1: 基础设置 */}
              <div className="p-3 rounded-2xl border space-y-2" style={{ backgroundColor: 'var(--control-soft-bg)', borderColor: 'var(--card-border)' }}>
                <label className="font-bold opacity-70 block">沙龙名称</label>
                <input
                  type="text"
                  value={title}
              ...
              <div className="p-3 rounded-2xl border space-y-2" style={{ backgroundColor: 'var(--control-soft-bg)', borderColor: 'var(--card-border)' }}>
                <div className="flex items-center justify-between">
                  <label className="font-bold opacity-70">聊天背景图</label>
              ...
              <div className="p-3 rounded-2xl border space-y-2" style={{ backgroundColor: 'var(--control-soft-bg)', borderColor: 'var(--card-border)' }}>
                <div className="flex items-center gap-1 font-bold opacity-70">
                  <Wand2 className="w-3.5 h-3.5" /> 气泡 CSS 快捷预设
                </div>
                <div className="grid grid-cols-2 gap-1.5 pt-1">
                  {CSS_PRESETS.map((preset, idx) => (
                    <button
                      ...
                      className="p-1.5 rounded-xl border text-[10px] font-medium text-left truncate hover:border-[var(--accent-color)]"
                      style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
                    >
                      {preset.name}
                    </button>
                ...
                <textarea
                  ...
                  className="w-full p-2 rounded-xl border outline-none font-mono text-[10px] mt-1"
                  style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
                />
              </div>
          ...
          {activeTab === 'persona' && (
            <div className="p-3 rounded-2xl border space-y-3" style={{ backgroundColor: 'var(--control-soft-bg)', borderColor: 'var(--card-border)' }}>
              <div>
                <label className="font-bold opacity-70 block mb-1">我的名字</label>
                <input
                  type="text"
                  value={userName}
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="font-bold opacity-70">我的头像</label>
                  <label className="text-[10px] font-bold px-2 py-0.5 rounded-lg border cursor-pointer flex items-center gap-1" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
              ...
              <div>
                <label className="font-bold opacity-70 block mb-1">我在本群的人设描述</label>
                <textarea
                  rows={3}
                  value={userPersona}
          ...
          {activeTab === 'members' && (
            <div className="space-y-3">
              <div className="space-y-1.5">
                {members.map((m, idx) => (
                  <div key={m.id || idx} className="flex items-center justify-between p-2 rounded-2xl border" style={{ backgroundColor: 'var(--control-soft-bg)', borderColor: 'var(--card-border)' }}>
                    <div className="flex items-center gap-2 overflow-hidden">
                      <div className="w-7 h-7 rounded-full overflow-hidden shrink-0 font-bold text-xs flex items-center justify-center" style={{ backgroundColor: 'var(--card-bg)' }}>
              ...
              <form onSubmit={handleSaveMember} className="p-3 rounded-2xl border space-y-2" style={{ backgroundColor: 'var(--control-soft-bg)', borderColor: 'var(--card-border)' }}>
                ...
                <div className="flex items-center gap-1">
                  <input
                    ...
                    style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
                  />
                  <label className="p-2 rounded-xl border cursor-pointer shrink-0" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
                ...
                <textarea
                  ...
                  style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--card-border)' }}
                />
                <button type="submit" className="w-full py-1.5 rounded-xl font-bold flex items-center justify-center gap-1" style={{ backgroundColor: 'var(--accent-color)', color: 'var(--accent-foreground)' }}>
          ...
          {activeTab === 'summaries' && (
            <div className="space-y-3">
              ...
              >
                ...
                <span>{isGeneratingSummary ? 'AI 总结中...' : '生成新阶段总结'}</span>
              </button>

              <div className="space-y-2">
                {summaries.map((s) => (
                  <div key={s.id} className="p-2.5 rounded-2xl border space-y-1.5" style={{ backgroundColor: 'var(--control-soft-bg)', borderColor: 'var(--card-border)' }}>
                    {editingSummaryId === s.id ? (
                      ...
                      <>
                        ...
                        <div className="flex items-center justify-between pt-1 border-t opacity-50 text-[9px]" style={{ borderColor: 'var(--card-border)' }}>
        ...
        <div className="shrink-0 pt-2 border-t" style={{ borderColor: 'var(--card-border)' }}>
          <button
            ...
            className="w-full py-2.5 rounded-2xl font-bold flex items-center justify-center gap-1.5 shadow-sm"
            style={{ backgroundColor: 'var(--accent-color)', color: 'var(--accent-foreground)' }}
          >
            <Check className="w-4 h-4" /> 保存并应用
          </button>
        </div>
      </div>
    </div>
